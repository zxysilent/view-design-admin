<template>
	<div>
			...
			<div style="max-width:520px">
				...
				<Form ref="dataForm" :model="dataForm" :label-width="100" label-position="right" :rules="dataRules">
					<FormItem label="登录账号：" prop="num">
						<Input :readonly="readonly" :disabled="readonly" v-model="dataForm.num"> </Input>
					</FormItem>
					...
					<FormItem>
						...
						<Button type="success" @click="load()" style="margin-left: 8px">重置填写</Button>
					</FormItem>
				</Form>
			</div>
		</Card>
		<Modal v-model="modalPass" :closable='false' :mask-closable=false :width="500">
			...
			<div slot="footer">
				<Button type="warning" :loading="loadingPass" @click="submitPass">提交保存</Button>
...
// import { admAuth, admEditInfo, admEditPass } from "@/api/auth";
export default {
	data() {
		return {
			dataForm: { avatar: "", city: "", ctime: "", id: 1, name: "", nick: "", num: "", phone: "" },
			loading: false,
	...
	methods: {
		...
		valideNum(rule, value, callback) {
			console.log(this.initNum, value);
			if (this.readonly || this.initNum == value) {
				callback();
			}
		...
		load() {
			admAuth().then(resp => {
				if (resp.code == 200) {
					...
					this.initNum = resp.data.num.slice(0);
					this.readonly = !(resp.data.role == 256);
				} else {
					this.$Message.error({
						content: `未查询到数据，请重试`,
						duration: 3
		...
		submitEdit() {
			this.$refs.dataForm.validate(valid => {
				if (valid) {
					this.loading = true;
					admEditInfo(this.dataForm).then(resp => {
						this.loading = false;
						if (resp.code == 200) {
