		...
		<Row class="bg" :style="{background:'url('+bgUrl+')'}">
			...
			<Card style="margin: 100px auto;max-width: 600px;padding-top: 20px;text-align: left">
				<div v-if="token">
					...
					<Form ref="dataForm" :model="dataForm" :rules="dataRules" :label-width="130">
						<Form-item label="类型" prop="type_id">
							<Radio-group v-model="dataForm.type_id">
						...
						<Form-item label="企业名称" prop="company">
							<Input v-model="dataForm.company" placeholder="请输入"></Input>
						</Form-item>
						<small class="help">标示即为公司英文简称</small>
						<Form-item label="标示" prop="mark">
							<Input v-model="dataForm.mark" placeholder="请输入"></Input>
						</Form-item>
						<small class="help">请输入15位营业执照号或18位的统一社会信用代码</small>
						<Form-item label="营业执照注册号" prop="companyId">
							<Input v-model="dataForm.companyId" placeholder="请输入"></Input>
						</Form-item>

						<Form-item label="管理员姓名" prop="userName">
							<Input v-model="dataForm.userName" placeholder="请输入"></Input>
						</Form-item>
						<Form-item label="管理员身份证号" prop="idCard">
							<Input v-model="dataForm.idCard" placeholder="请输入"></Input>
						</Form-item>
						<Form-item label="管理员手机号" prop="phone">
							<Input v-model="dataForm.phone" placeholder="请输入"></Input>
						</Form-item>
						<Form-item>
							<Button type="primary" @click="handleSubmit('dataForm')" long :loading="loading">提交
							</Button>
						</Form-item>
					</Form>
				</div>
				...
				<small style="display: block;text-align: center;line-height: 30px;"><img :src="require('@/assets/img/logo.png')" height="30" alt="" style="vertical-align: top"><span>Copyright by xxxxx ,All rights reserved</span>
				</small>
			</Card>
			</Col>
		</Row>
	...
	methods: {
		handleSubmit(name) {
			...
			this.$refs[name].validate(valid => {
				if (valid) {
					let data = {
						account_type: this.dataForm.type_id,
						company_name: this.dataForm.company,
						business_license_number: this.dataForm.companyId,
						personName: this.dataForm.userName,
						personSex: this.dataForm.idCard.substring(16, 1) % 2 ? "1" : "0",
						idCard: this.dataForm.idCard,
						telephone: this.dataForm.phone,
						promotionTableName: "",
						promotionTableId: "",
						email: ""
					};
